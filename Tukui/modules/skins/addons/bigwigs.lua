...
if not C["Addon_Skins"].Bigwigs == true or not IsAddOnLoaded("BigWigs") then return end
...
local applystyle = function(bar)
	...
	bg:Show()
	bar:Set("bigwigs:tukui:barbg", bg)
	...
	if bar.candyBarIconFrame:GetTexture() then
		...
		ibg:Show()
		bar:Set("bigwigs:tukui:iconbg", ibg)
	end
...
local function RegisterStyle()
	...
	if prox and bars.db.profile.barStyle == "TukUi" then
...
local function PositionBWAnchor()
	...
	if E.CheckAddOnShown() == true then
		...
		BigWigsAnchor:Point("BOTTOMRIGHT", T.UIParent, "BOTTOMRIGHT", -5, 8)		
	end
end
